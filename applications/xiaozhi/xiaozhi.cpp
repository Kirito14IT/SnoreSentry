...
/* ── Decibel monitor WebSocket connection ── */
static err_t my_decibel_wsapp_fn(int code, char *buf, size_t len)
{
    (void)buf; (void)len;
    switch (code)
    {
    case WS_CONNECT:
        LOG_I("Decibel WebSocket connected\n");
        g_app.decibel_ws.is_connected = 1;
        if (g_app.decibel_ws.sem) rt_sem_release(g_app.decibel_ws.sem);
        break;
    case WS_DISCONNECT:
        LOG_I("Decibel WebSocket disconnected\n");
        g_app.decibel_ws.is_connected = 0;
        break;
    }
    return 0;
}
...
void decibel_ws_connect(void)
{
    ...
    while (retry-- > 0)
    {
        ...
        if (!check_internet_access())
        {
            LOG_I("Waiting internet for decibel WS...\n");
            rt_thread_mdelay(500);
            continue;
        }

        /* Cleanup existing connection */
        if (g_app.decibel_ws.clnt.pcb != RT_NULL && ((struct tcp_pcb *)g_app.decibel_ws.clnt.pcb)->state != CLOSED)
        {
            wsock_close(&g_app.decibel_ws.clnt, WSOCK_RESULT_LOCAL_ABORT, ERR_OK);
            rt_thread_mdelay(200);
        }

        if (!g_app.decibel_ws.sem)
            g_app.decibel_ws.sem = rt_sem_create("db_ws", 0, RT_IPC_FLAG_FIFO);

        if (!g_app.decibel_ws.ws_write_mutex)
            g_app.decibel_ws.ws_write_mutex = rt_mutex_create("db_ws_write", RT_IPC_FLAG_FIFO);

        wsock_init(&g_app.decibel_ws.clnt, 1, 1, my_decibel_wsapp_fn);
        err = wsock_connect(&g_app.decibel_ws.clnt, MAX_WSOCK_HDR_LEN,
                            DECIBEL_HOST, DECIBEL_WSPATH,
                            DECIBEL_PORT, XIAOZHI_TOKEN, NULL,
                            "Protocol-Version: 1\r\n");
        LOG_I("Decibel WS connect: %d\n", err);

        if (err == 0 && rt_sem_take(g_app.decibel_ws.sem, 3000) == RT_EOK)
        {
            if (g_app.decibel_ws.is_connected)
            {
                LOG_I("Decibel WebSocket connected successfully\n");
                return;
            }
        }
        LOG_E("Decibel WS retry, remaining: %d\n", retry);
        rt_thread_mdelay(1000);
    }
    LOG_E("Decibel WebSocket connection failed\n");
}
...
void decibel_ws_send(uint8_t *data, int len)
{
    ...
    if (g_app.decibel_ws.ws_write_mutex && rt_mutex_take(g_app.decibel_ws.ws_write_mutex, RT_WAITING_NO) == RT_EOK)
    {
        if (g_app.decibel_ws.is_connected)
        {
            err_t err = wsock_write(&g_app.decibel_ws.clnt, (const char *)data, len, OPCODE_BINARY);
            if (err != ERR_OK)
            {
                LOG_D("decibel_ws_write failed: %d\n", err);
                g_app.decibel_ws.is_connected = 0;
            }
        }
        rt_mutex_release(g_app.decibel_ws.ws_write_mutex);
    }
}
